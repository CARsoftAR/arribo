...
class _MapScreenState extends State<MapScreen> with TickerProviderStateMixin {
    ...
    final filtered = activeFilter.isEmpty 
        ...
        : _allVehicles.where((v) {
            ...
            if (_activeSearchedLine != null && _lineRamales.containsKey(_activeSearchedLine)) {
              if (name.contains(_activeSearchedLine!.toLowerCase())) {
                return _selectedRamales.any((selectedBranch) {
                  ...
                  if (branchNameClean.contains('l azul') && (name.contains('l azul') || name.endsWith('159') || v.destination.toLowerCase().contains('azul') || v.destination.toLowerCase().contains('alpargatas'))) return true;
  ...
  Future<List<LatLng>> _loadLocalJsonFallback(String line, String shapeId) async {
    try {
      ...
      if (line.contains('159')) {
        if (shapeId.contains('r1')) {
          assetPath = 'assets/data/route_159_r1_detailed.json';
        } else if (shapeId.contains('r2')) {
          assetPath = 'assets/data/route_159_r2_detailed.json';
        } else if (shapeId.contains('azul')) {
          assetPath = 'assets/data/route_159_azul_detailed.json';
        } else if (shapeId.contains('roja')) {
          assetPath = 'assets/data/route_159_roja_detailed.json';
        }
      } else if (line.contains('98')) {
        if (shapeId.contains('r3')) {
          assetPath = 'assets/data/route_98_r3_detailed.json';
        } else if (shapeId.contains('r5')) {
          assetPath = 'assets/data/route_98_r5_detailed.json';
        }
      }
      ...
      debugPrint('[MapScreen] Failed loading local JSON fallback: $e');
    }
    return [];
  }
        ...
        if (points.isNotEmpty) {
          ...
          if (shapeId.contains('98')) {
            routeColor = shapeId.contains('r3')
                ? const Color(0xFF137A3E)
                : const Color(0xFFE11D48);
          } else {
            if (shapeId.contains('r1')) {
              routeColor = const Color(0xFF0EA5E9);
            } else if (shapeId.contains('r2')) {
              routeColor = const Color(0xFF8B5CF6);
            } else if (shapeId.contains('azul')) {
              routeColor = const Color(0xFF3B82F6);
            } else if (shapeId.contains('roja')) {
  ...
  void _showRouteFor(TransitVehicle vehicle) async {
    try {
      ...
      if (points.isNotEmpty) {
        ...
        newPolylines.add(
          Polyline(
            polylineId: PolylineId('route_${vehicle.id}_$shapeId'),
            points: points,
            color: routeColor.withValues(alpha: 0.95),
            width: 5,
            jointType: JointType.round,
      ...
      debugPrint('[MapScreen] Error rendering vehicle route: $e');
    }
  }
